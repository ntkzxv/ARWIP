...
export default function CollectionHistoryView() {
  ...
  return (
    <div className="flex flex-col h-full max-w-[1400px] mx-auto w-full px-4 md:px-8 pt-10 pb-10">
      <div className="flex-1 space-y-5">
        {loading ? (
          // 🚩 โชว์ Skeleton ขณะโหลด
          <div className="space-y-6 animate-pulse">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="bg-white p-8 rounded-[35px] border border-slate-100 flex items-center justify-between">
                ...
                <div className="flex gap-10 items-center">
                  <div className="h-8 bg-slate-50 rounded-xl w-32" />
                  <div className="w-11 h-11 bg-slate-50 rounded-full" />
                </div>
              </div>
            ))}
          </div>
          ...
          currentItems.map((group, index) => {
            ...
            return (
              <div key={group.contract_id} className="relative group transition-all duration-500 animate-in fade-in duration-700">
                {/* --- 👤 Card Header --- */}
                <div 
                  onClick={() => setExpandedContractId(isExpanded ? null : group.contract_id)}
                  className={`relative z-10 bg-white p-6 md:p-8 rounded-[35px] border transition-all duration-500 cursor-pointer flex flex-col md:flex-row md:items-center justify-between gap-6 ${
                    isExpanded 
                ...
                >
                  <div className="flex items-center gap-5">
                    ...
                    <div className="min-w-0">
                      ...
                      {hasUrgentAction && (
                          <span className="flex items-center gap-1 bg-rose-500 text-white text-[9px] font-black px-2 py-0.5 rounded-full uppercase animate-pulse italic">
                            <Zap size={10} fill="white" /> กำลังติดตามด่วน
                          </span>
                      )}
                      {group.current_overdue >= 3 && (
                          <span className="bg-rose-100 text-rose-600 text-[9px] font-black px-2 py-0.5 rounded-full border border-rose-200 uppercase italic">
                              Overdue x{group.current_overdue}
                          </span>
                      )}
                  ...
                  <div className="mx-4 md:mx-10 bg-slate-50/80 rounded-b-[40px] -mt-10 pt-16 pb-8 px-6 md:px-12 animate-in slide-in-from-top-8 duration-700 border-x-2 border-b-2 border-slate-100 shadow-inner">
                      ...
                      <div className="space-y-4">
                          {group.logs.map((log: any, logIdx: number) => {
                            const isUrgent = log.result_note.includes('[แจ้งเตือน SMS]') || log.result_note.includes('[ติดตามด่วน]');
                            ...
                            return (
                                <div key={log.id} className="relative flex gap-4 group/log">
                                  ...
                                  <div className={`relative z-10 shrink-0 w-10 h-10 rounded-xl flex items-center justify-center shadow-sm ${
                                      isUrgent ? 'bg-rose-500 text-white' : 'bg-white text-slate-400 border border-slate-200'
                                  }`}>
                                      {isUrgent ? <Zap size={16} fill="white" /> : <MessageSquare size={16} />}
                                  </div>
                                  ...
                                  <div className="flex-1 bg-white p-5 rounded-2xl shadow-sm border border-slate-100 group-hover/log:border-slate-300 transition-colors">
                                      ...
                                      <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-md italic ${
                                          isUrgent ? 'bg-rose-50 text-rose-600' : 'bg-slate-50 text-slate-500'
                                      }`}>
                                          ค้างชำระงวดที่ {log.installment_payments?.installment_number}
                                      </span>
                    ...
                    <div className="pt-6 mt-4 border-t border-slate-200 flex justify-end">
                      <Link 
                          href={`/datacenter/customers/${group.customer_id}?tab=purchase&contractId=${group.contract_id}`}
                          className="group/btn flex items-center gap-3 bg-slate-900 text-white px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-black transition-all shadow-lg active:scale-95 italic"
                      >
                          เช็คประวัติการซื้อ 
          ...
          <div className="py-32 text-center flex flex-col items-center justify-center gap-6 bg-slate-50/50 rounded-[40px] border-2 border-dashed border-slate-200">
             <div className="w-24 h-24 rounded-full bg-white flex items-center justify-center shadow-sm text-slate-200">
                <UserCheck size={48} />
             </div>
             <p className="font-black uppercase tracking-[0.3em] text-xs text-slate-400 italic">Archive Empty</p>
          </div>
        )}
      </div>
    </div>
